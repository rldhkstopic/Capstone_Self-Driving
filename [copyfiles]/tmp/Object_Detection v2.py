...
def detect_from_cvmat(yolo, img):
    ...
    inputs = np.zeros((1, size, size, 3), dtype='float32')
...
def draw_background_highlight(image, draw_img, w):
    mask = cv2.rectangle(np.copy(image), (0, 0), (int(w/3), 100), (0, 0, 0), thickness=cv2.FILLED)
    draw_img = cv2.addWeighted(mask, 0.3, draw_img, 0.8, 0)
    return draw_img

def draw_results(img, image_lane, yolo, fps):
    img_cp = img.copy()
...
def vehicle_detection_yolo(image, image_lane):
    start = timer()
    fps = 1.0 / (timer() - start)
    detect_from_file(yolo, image)

    yolo_result = draw_results(image, image_lane, yolo, fps)
    return yolo_result
